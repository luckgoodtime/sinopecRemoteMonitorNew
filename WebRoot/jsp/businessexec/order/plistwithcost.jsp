		...
		function dataGridTable() {
			...
			$("#dataGridTable").datagrid({
			      ...
			      columns: [[
			        ...
			        { field: 'freightPrice', title: '运费单价'},
			        { field: 'transportFee', title: '运费'},
			        { field: 'purchaseCost', title: '采购总成本'},
			        { field: 'inStockFlag', title: '入库标记'}
			        
			        { field: 'CreateBy', title: '操作',
			        	formatter: function (val, row, index) {
		        		 var str = '<a href="<%=request.getContextPath()%>/biz/addOrUpdateCost.do?from=purchaseCostRegister&bizType=PURCHASE&id='+row.id+'" class="tablelink">修改</a>';
		        		 str += '&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/biz/del.do?from=purchaseCostRegister&bizType=PURCHASE&ids='+row.id+'" class="tablelink">删除</a>';
		        		 if(row.money && "已入库" != row.inStockFlag)
			        		 str += '&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/biz/instock.do?from=purchaseCostRegister&bizType=PURCHASE&id='+row.id+'" class="tablelink">入库</a>';
		                 return str;
			          }}
